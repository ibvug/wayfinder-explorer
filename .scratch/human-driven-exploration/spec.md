...
flowchart TD
    ...
    Chart --> Destination["探索者确认目的地"]
    Destination --> Scope["共同确认取证范围"]
    Scope --> Origin["探索者确认起点"]
...
探索者从模糊目标开始，与地图 Agent 澄清可观察的目标状态。地图 Agent 可以提问、调查和总结，但只有探索者明确确认后，目的地节点才成立。

目的地确认后保持稳定。如果抵达标准发生根本变化，应建立新的目标探索，而不是把现有目的地静默改写成另一件事。

### 2. 确认取证范围和起点

地图 Agent 在检查现有环境前与探索者明确取证范围。取证范围允许观察事实，不自动授予写入、发布或其他副作用权限。

地图 Agent 根据取证结果形成当前现状摘要和证据指针；探索者确认后形成固定的起点节点。后续环境变化不会静默重写这个历史基线。
...
当前项目已经具备大部分底层能力，包括持续探索会话、答案草案、写回预览、原子文件写入、答案历史、重新绘图校验、待重新绘图恢复和稳定地图节点投影。

仍需收拢的关键差距是：

1. HTTP 确认路由当前先调用 `ExpeditionManager.confirmWriteback`，再自行调用 `ChartingManager.rechartAfterConfirmation`。跨模块耐久顺序由传输层掌握。
2. 当前写回预览主要覆盖答案和直接地图写回；完整影响协调提案在确认后才由重新绘图流程形成，探索者无法在一个计划中看到候选答案的全部预计影响。
3. 当前队列主要是重新绘图队列，而不是从确认入口开始覆盖“排队、耐久确认、影响协调、恢复”的完整确认队列。
4. Journey 与 Charting 分别记录状态；答案已经确认但尚未耐久登记重新绘图请求时，跨模块交接仍可能留下恢复空隙。
5. 部分探索上下文更新可能在完整重新绘图提案通过全部领域校验和持久化前被消费，需要把“验证、规范写入、对外投影”的次序集中管理。
